...
module.exports.registerUser = asyncHandler(async (req, res, next) => {
  ...
  res.status(201).send({
    ...
    user: _.pick(user, ["_id", "email", "name", "token", "createdAt", "role"])
  });
});
...
module.exports.loginUser = asyncHandler(async (req, res, next) => {
  ...
  res
    .status(200)
    .send({ succuss: false, code: 201, message: "user loggedin", user: user });
});
...
module.exports.forgotPassword = asyncHandler(async (req, res, next) => {
  ...
  if (!user) {
    res.status(404).send({
      succuss: false,
      code: 404,
      message: `there is no user with this ${req.body.email}`
    });
  }
...
module.exports.resetPassword = asyncHandler(async (req, res, next) => {
  if (!req.body.oldPassword || !req.body.newPassword) {
    return res.status(400).send({
      success: false,
      code: 400,
      message: " please add oldPassword field and newPassword field"
    });
  } else if (
    req.body.oldPassword.length < 5 ||
    req.body.newPassword.length < 5
  ) {
    return res.status(400).send({
      success: false,
      code: 400,
      message: "oldPassword ,newPassword length of atleast 6 characters "
    });
  }
